...
export default function AmericanMahjongTable({ onWin }: { onWin?: (points: number) => void }) {
  ...
  const declare = () => {
    ...
    if (result.declared) { if (soundEnabled) playMahjongSound('win', undefined, 'english'); setGame(result.state); setNotice(t('mahJonggExclaim') + ' ' + result.evaluation.message); onWin?.(result.state.settlement?.points ?? 0); }
  ...
  const claim = (kind: 'pung' | 'kong') => {
    try {
      ...
      if (soundEnabled) playMahjongSound(kind === 'pung' ? 'pon' : 'kan', undefined, 'english'); setGame(next); setNotice(t('youCalled', { kind }));
  ...
  const claimMahJongg = () => {
    try {
      ...
      if (soundEnabled) playMahjongSound('win', undefined, 'english'); setGame(result.state); setNotice(t('mahJonggExclaim') + ' ' + result.evaluation.message); onWin?.(result.state.settlement?.points ?? 0);
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Mah Jongg claim failed.'); }
  };
  const passClaims = () => {
    try {
      const next = passAmericanClaims(game);
      setGame(next); setNotice(describeTableTurn(next, t));
  ...
  const swapJoker = (meldIndex: number) => {
    ...
    try {
      ...
      setGame(next); setJokerExchangeMeld(null); setNotice(t('jokerExchanged'));
  ...
  const discard = (index: number) => {
    ...
    try {
      ...
      primeMahjongAudio(); if (soundEnabled) playMahjongSound('discard', tile as Tile, 'english'); setGame(next); setLastDiscard(tile); setNotice(describeTableTurn(next, t));
    ...
    <section ref={tableRef} data-high-contrast={preferences.highContrast} data-reduced-motion={preferences.reducedMotion} data-tile-scale={preferences.tileScale} className={`mahjong-table-shell ${isFullscreen ? 'mahjong-table-shell--fullscreen' : ''} overflow-hidden rounded-xl bg-[#176845] p-0 shadow-[0_24px_60px_rgba(0,45,31,.35)] lg:p-3`}>
      ...
      <div className="mahjong-desktop-shell hidden min-w-[700px] min-[700px]:block" style={isFullscreen ? { display: 'flex', flexDirection: 'column', height: 'calc(100dvh - 16px)' } : undefined}>
        ...
        <div className="mahjong-desktop-board mahjong-desktop-board--seasonal relative h-[720px] overflow-hidden border-[5px] border-[#032f22] bg-[#00553e] shadow-[inset_0_0_90px_rgba(0,30,22,.34)]" style={isFullscreen ? { height: 'auto', minHeight: 0, flex: '1 1 0%' } : undefined}>
          ...
          {game.players[0].melds.length > 0 && <div className="absolute left-[16%] top-[67%] z-30 rounded-lg bg-black/35 p-2 text-xs text-emerald-50"><p className="mb-1 font-black">{t('yourExposedGroups')}</p>{game.players[0].melds.map((meld, index) => <div key={index} className="mb-1 flex items-center gap-1">{meld.tiles.map((tile, tileIndex) => <AmericanTile key={tileIndex} tile={tile} compact />)}{meld.jokerIndexes.length > 0 && <button type="button" onClick={() => setJokerExchangeMeld(index)} className="rounded bg-amber-300 px-2 py-1 font-black text-emerald-950">{t('exchangeJoker')}</button>}</div>)}</div>}
      ...
      <div className="min-h-[620px] bg-[radial-gradient(circle_at_center,#087052_0%,#00553e_62%,#003c2d_100%)] p-3 text-white min-[700px]:hidden" style={isFullscreen ? { minHeight: '100dvh' } : undefined}>
        ...
        {game.players[0].melds.length > 0 && <div className="mt-3 rounded-xl bg-black/20 p-3 text-xs"><strong>{t('exposedGroups')}</strong>{game.players[0].melds.map((meld, index) => <div key={index} className="mt-2 flex items-center gap-1">{meld.tiles.map((tile, tileIndex) => <AmericanTile key={tileIndex} tile={tile} compact />)}{meld.jokerIndexes.length > 0 && <button type="button" onClick={() => setJokerExchangeMeld(index)} className="rounded bg-amber-300 px-2 py-1 font-black text-emerald-950">{t('exchangeJoker')}</button>}</div>)}</div>}
      </div>
      {jokerExchangeMeld !== null && exchangeMeld && exchangeNatural && <div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/65 p-4">
        <div className="w-full max-w-md rounded-2xl border-2 border-amber-200 bg-[#f5f0df] p-5 text-emerald-950 shadow-2xl">
          ...
          <div className="mt-4 flex gap-2"><button type="button" onClick={() => setJokerExchangeMeld(null)} className="flex-1 rounded-xl border border-emerald-900/30 py-3 font-black">{t('cancel')}</button><button type="button" disabled={!canConfirmJokerExchange} onClick={() => swapJoker(jokerExchangeMeld)} className="flex-1 rounded-xl bg-amber-300 py-3 font-black disabled:cursor-not-allowed disabled:opacity-45">{t('exchange')}</button></div>
        </div>
      </div>}
...
function ResultReview({ game, onNew }: { game: AmericanGameState; onNew: () => void }) {
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/65 p-4">
    <div className="max-h-[88vh] w-full max-w-3xl overflow-auto rounded-2xl border-4 border-amber-200 bg-[#f5f0df] p-5 text-emerald-950 shadow-2xl">
      ...
      <button type="button" onClick={onNew} className="mt-4 w-full rounded-xl bg-[#087052] py-3 font-black text-white">{t('newHand')}</button>
    </div>
  </div>;
}
