...
/** P0: never surface Coming Soon skins on the wardrobe page. */
function isWardrobeVisible(id: AppearanceId) {
...
export default function Wardrobe() {
  ...
  const visibleIds = useMemo(
    () => (Object.keys(APPEARANCES) as AppearanceId[]).filter(isWardrobeVisible),
    []
  );

  useEffect(() => {
    const active = savedAppearance();
    const next = isWardrobeVisible(active) ? active : 'jade';
    setSelected(next);
    setEquipped(next);
  }, []);
    ...
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <section className="overflow-hidden rounded-3xl border border-[#d8d7cd] bg-[#fffdf7] p-4 sm:p-6">
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-[#52617a]">{t('freeNote')}</p>
        </div>
        <div className="space-y-6">
          {CHAPTERS.map((chapter) => {
            const ids = visibleIds.filter((id) => APPEARANCES[id].chapter === chapter);
            if (ids.length === 0) return null;
            return (
              <section key={chapter} aria-labelledby={`chapter-${chapter}`}>
                <div className="mb-3">
                  <h3 id={`chapter-${chapter}`} className="font-serif text-lg font-bold text-[#1d2a44]">
                    {t(`chapters.${chapter}.title`)}
                  </h3>
                  <p className="mt-0.5 text-xs text-[#52617a]">{t(`chapters.${chapter}.body`)}</p>
                </div>
      ...
      <aside className="rounded-3xl border border-[#d8d7cd] bg-[#fffdf7] p-5">
        ...
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(rgba(16,62,53,.52),rgba(16,62,53,.58)),url('${APPEARANCES[selected].table}')`
          }}
        >
        ...
        <div className="mt-4 flex items-center gap-3 rounded-2xl bg-[#f3efe5] p-3">
          <span
            ...
            style={{ backgroundImage: `url('${APPEARANCES[selected].back}')` }}
          />
          <p className="text-sm text-[#52617a]">
            {selected === equipped ? t('equipped') : t('owned')}
          </p>
        </div>
        <button
          type="button"
          ...
          className="mt-4 w-full rounded-xl bg-[#1e554d] px-4 py-3 text-sm font-bold text-white shadow-sm hover:bg-[#2d756a]"
        >
          {t('apply')}
        </button>
      </aside>
