...
export const GAME_I18N: Record<string, GameI18n> = {
  ...
  'taiwan-mahjong': {
    ...
    content: {
      zh: {
        intro: 'Mahjong Hub 台湾麻将采用轻松的 16 张入门规则：使用 144 张牌，花牌与季节牌须即时补牌，以五副面子加一对将牌胡；台数可累加，本桌允许 0 台胡。',
        howToPlay: ['使用 144 张牌。闲家起手 16 张，庄家起手 17 张并先打牌。', '摸到花牌或季节牌须立即亮出并从牌墙补一张；花牌不计入 16 张手牌。', '轮到自己时摸一张、打出一张；可依规则吃、碰、杠。', '五副面子加一对将牌组成 17 张即可胡；支持自摸与放铳胡。', '台型相加结算；本站入门桌允许完整的 0 台牌胡。'],
        tips: ['16 张手牌应尽早保留多个相连搭子，不要过早只押一种听牌。', '花牌能增加价值，但不能代替主手牌的五副面子结构。', '台湾麻将杠牌较多，杠后补牌与公开信息都需要纳入判断。'],
        faq: [{ question: '为什么起手有 16 张？', answer: '台湾麻将由五副面子和一对将牌组成；玩家通常持有 16 张，在摸到或吃到第 17 张时胡牌。' }, { question: '0 台可以胡吗？', answer: '可以。本桌采用适合新手的 0 台基线；其他地区的台湾麻将桌可能要求最低台数。' }]
      },
      'zh-TW': { intro: 'Mahjong Hub 台灣麻將採用輕鬆的 16 張入門規則：144 張牌、花季即時補牌、五組面子加一對將牌，並採可累加台數與 0 台可胡。' },
      ja: { intro: 'Mahjong Hub の台湾麻雀は、144枚・花牌補充・5面子1雀頭を使う初心者向け16枚ルールです。台数は加算し、0台でも和了できます。' },
      ko: { intro: 'Mahjong Hub 대만 마작은 144장, 꽃패 보충, 다섯 멘츠와 한 쌍을 사용하는 입문용 16장 규칙이며 0타이 화료를 허용합니다.' }
    }
  },
  'american-mahjong': {
